...
export default function StatutoryComponents({ onComplete }: StatutoryComponentsProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 m-2 sm:m-4 md:m-6 w-[calc(100%-1rem)] sm:w-[calc(100%-2rem)] md:w-[calc(100%-3rem)] bg-white p-4 sm:p-6 rounded-lg border border-gray-200">
      <h2 className="text-xl sm:text-2xl font-semibold text-gray-900">Statutory Components</h2>

      {/* Tabs */}
      <div className="border-b border-gray-200">
        <div className="overflow-x-auto scrollbar-hide">
          <nav className="flex space-x-4 sm:space-x-6 md:space-x-8 min-w-max" aria-label="Tabs">
            {tabs.map((tab) => (
              <button
                key={tab.id}
